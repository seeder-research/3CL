...
var (
	...
	// TODO: lex2, din2 no longer depends on Msat -> remove child dependency
...
func AddExchangeField(dst *data.Slice) {
	...
	case bulk && !inter:
		...
		opencl.AddDMIBulk(dst, M.Buffer(), lex2.Gpu(), dbulk2.Gpu(), regions.Gpu(), M.Mesh()) // dmi+exchange
	case inter && bulk:
...
func (p *dexchParam) Gpu() opencl.SymmLUT {
	p.update()
	if !p.gpu_ok {
		p.upload()
	}
...
func (p *aexchParam) Gpu() opencl.SymmLUT {
	p.update()
	if !p.gpu_ok {
		p.upload()
	}
