...
func golden_fft3(dataIn []float32, N int) []float32 {
	arr := dataIn
	...
	for i := 0; i < N; i++ {
		...
		out_arr[idx] = in0_r + in1_r + in2_r
		out_arr[idx+1] = in0_i + in1_i + in2_i
		out_arr[idx+2*N]   = in0_r + (cs1*in1_r + sn1*in1_i) + (cs1*in2_r - sn1*in2_i)
		out_arr[idx+2*N+1] = in0_i + (cs1*in1_i - sn1*in1_r) + (cs1*in2_i + sn1*in2_r)
		out_arr[idx+4*N]   = in0_r + (cs1*in1_r - sn1*in1_i) + (cs1*in2_r + sn1*in2_i)
		out_arr[idx+4*N+1] = in0_i + (cs1*in1_i + sn1*in1_r) + (cs1*in2_i - sn1*in2_r)
	}
	return out_arr
}
...
func main() {
	...
	kernelObj := kernels["fft3_c2c_long_interleaved_oop"]
	...
	fmt.Printf("Begin first run of fft3_c2c_long_interleaved_oop kernel... \n");
	...
	fmt.Printf("First run of fft3_c2c_long_interleaved_oop kernel completed and starting inverse to check... \n");

	kernelObj1 := kernels["ifft3_c2c_long_interleaved_oop"]
	...
	fmt.Printf("Begin first run of ifft3_c2c_long_interleaved_oop kernel... \n");
	...
	fmt.Printf("Finished tests on fft3_c2c_long_interleaved_oop\n")
