...
func golden_fft8(dataIn []float32, N int) []float32 {
	arr := dataIn
	...
	for i := 0; i < N; i++ {
		...
		out_arr[idx]        = x0_r_0 + x0_r_1
		out_arr[idx+1]      = x0_i_0 + x0_i_1
		out_arr[idx+2*N]    = x1_r_0 + float32(math.Sqrt(0.5))*(x1_r_1 + x1_i_1)
		out_arr[idx+2*N+1]  = x1_i_0 + float32(math.Sqrt(0.5))*(x1_i_1 - x1_r_1)
		out_arr[idx+4*N]    = x2_r_0 + x2_i_1
		out_arr[idx+4*N+1]  = x2_i_0 - x2_r_1
		out_arr[idx+6*N]    = x3_r_0 + float32(math.Sqrt(0.5))*(x3_i_1 - x3_r_1)
		out_arr[idx+6*N+1]  = x3_i_0 - float32(math.Sqrt(0.5))*(x3_r_1 + x3_i_1)
		out_arr[idx+8*N]    = x0_r_0 - x0_r_1
		out_arr[idx+8*N+1]  = x0_i_0 - x0_i_1
		out_arr[idx+10*N]   = x1_r_0 - float32(math.Sqrt(0.5))*(x1_r_1 + x1_i_1)
		out_arr[idx+10*N+1] = x1_i_0 - float32(math.Sqrt(0.5))*(x1_i_1 - x1_r_1)
		out_arr[idx+12*N]   = x2_r_0 - x2_i_1
		out_arr[idx+12*N+1] = x2_i_0 + x2_r_1
		out_arr[idx+14*N]   = x3_r_0 - float32(math.Sqrt(0.5))*(x3_i_1 - x3_r_1)
		out_arr[idx+14*N+1] = x3_i_0 + float32(math.Sqrt(0.5))*(x3_r_1 + x3_i_1)
	}
	return out_arr
}
...
func main() {
	...
	kernelObj := kernels["fft8_c2c_long_interleaved_oop"]
	...
	fmt.Printf("Begin first run of fft8_c2c_long_interleaved_oop kernel... \n");
	...
	if err := kernelObj.SetArgs(input, output, uint32(testSz/16), uint32(1), uint32(8)); err != nil {
	...
	fmt.Printf("First run of fft8_c2c_long_interleaved_oop kernel completed and starting inverse to check... \n");

	kernelObj1 := kernels["ifft8_c2c_long_interleaved_oop"]
	...
	if err = kernelObj1.SetArgs(output, rev_in, uint32(testSz/16), uint32(1), uint32(8)); err != nil {
		fmt.Printf("SetKernelArgs failed: %+v \n", err)
		return
	}
	...
	fmt.Printf("Begin first run of ifft8_c2c_long_interleaved_oop kernel... \n");
	...
	fmt.Printf("Finished tests on fft8_c2c_long_interleaved_oop\n")
