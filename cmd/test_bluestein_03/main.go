...
func main() {
	...
	if (dataSize < 4) || (dataSize > 18) {
		fmt.Println("argument to -length must be an integer from 4 to 18!")
		os.Exit(-1)
	}
	dataSize = int(math.Exp2(float64(dataSize)))
	// tol := 5e-7
	...
	_, errFI = fftPlanHandleI.EnqueueBackwardTransform([]*cl.CommandQueue{queue}, nil, []*cl.MemObject{bufX}, []*cl.MemObject{bufOut}, nil)
	if errFI != nil {
	...
	_, errFI = queue.EnqueueReadBufferFloat32(bufOut, true, 0, X, nil)
	errFI = queue.Finish()
	if errFI != nil {
	...
	fmt.Printf("\n")
	fmt.Printf("Performing FFT to confirm... \n")

	/********** Set up FFT to check golden results **********/
	...
	_, errF = fftPlanHandle.EnqueueBackwardTransform([]*cl.CommandQueue{queue}, nil, []*cl.MemObject{bufOut}, []*cl.MemObject{bufX}, nil)
	if errF != nil {
		fmt.Printf("unable to enqueue transform: %+v \n", errF)
	}
	...
	/********** Fetch result of the FFT plan **********/
	_, errF = queue.EnqueueReadBufferFloat32(bufX, true, 0, X, nil)
	errF = queue.Finish()
	if errFI != nil {
		fmt.Printf("unable to read output buffer: %+v \n", errF)
	}

	/********** Print result of the FFT to screen **********/
	fmt.Printf("Returned FFT result: \n")
	print_iter = 0
	for print_iter < dataSize {
		fmt.Printf("(%f, %f) ", float32(dataSize)*X[2*print_iter], float32(dataSize)*X[2*print_iter+1])
		print_iter++
	}
	fmt.Printf("\n")
	/********** First run the inverse FFT using clFFT is complete **********/
	...
	if _, err := queue.EnqueueWriteBufferFloat32(bufA, true, 0, inputs0[:], nil); err != nil {
		fmt.Printf("failed to write data into buffer \n")
	}
	errF = queue.Finish()
	if errF != nil {
	...
	errFI = fftPlanHandleI.SetScale(cl.ClFFTDirectionBackward, float32(float64(1.0)/float64(bluestein_len*dataSize)))
	if errFI != nil {
		fmt.Printf("unable to set fft result location \n")
	}
