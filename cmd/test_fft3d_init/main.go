...
func main() {
	...
	bufX, errC := context.CreateEmptyBuffer(cl.MemWriteOnly, N0*N1*N2*2*int(unsafe.Sizeof(X[0])))
	bufOut, errCO := context.CreateEmptyBuffer(cl.MemReadOnly, N0*N1*N2*2*int(unsafe.Sizeof(X[0])))
