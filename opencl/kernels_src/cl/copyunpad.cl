...
copyunpad(__global float* __restrict  dst, int Dx, int Dy, int Dz,
	...
	int ix = get_group_id(0) * get_loacl_size(0) + get_local_id(0);
	int iy = get_group_id(1) * get_loacl_size(1) + get_local_id(1);
	int iz = get_group_id(2) * get_loacl_size(2) + get_local_id(2);
