...
func kernMulRSymm3D_async(fftM [3]*data.Slice, Kxx, Kyy, Kzz, Kyz, Kxz, Kxy *data.Slice, Nx, Ny, Nz int) {
	...
	event := k_kernmulRSymm3D_async(fftM[X].DevPtr(0), fftM[Y].DevPtr(0), fftM[Z].DevPtr(0),
		...
		Nx, Ny, Nz, cfg, []*cl.Event{fftM[X].GetEvent(0), fftM[Y].GetEvent(0), fftM[Z].GetEvent(0),
	...
	if err != nil { fmt.Printf("WaitForEvents failed in kernmulrsymm3d_async: %+v \n", err) }
}
...
func kernMulRSymm2Dxy_async(fftMx, fftMy, Kxx, Kyy, Kxy *data.Slice, Nx, Ny int) {
	...
	event := k_kernmulRSymm2Dxy_async(fftMx.DevPtr(0), fftMy.DevPtr(0),
		...
		Nx, Ny, cfg, []*cl.Event{fftMx.GetEvent(0), fftMy.GetEvent(0), Kxx.GetEvent(0), Kyy.GetEvent(0), Kxy.GetEvent(0)})
	...
	Kxy.SetEvent(0, event)
	err := cl.WaitForEvents([](*cl.Event){event})
	if err != nil { fmt.Printf("WaitForEvents failed in kernmulrsymm2dxy_async: %+v \n", err) }
}
...
func kernMulRSymm2Dz_async(fftMz, Kzz *data.Slice, Nx, Ny int) {
	...
	cfg := make3DConf([3]int{Nx, Ny, 1})
	event := k_kernmulRSymm2Dz_async(fftMz.DevPtr(0), Kzz.DevPtr(0), Nx, Ny, cfg, []*cl.Event{fftMz.GetEvent(0), Kzz.GetEvent(0)})
	fftMz.SetEvent(0, event)
	Kzz.SetEvent(0, event)
	err := cl.WaitForEvents([]*cl.Event{event})
	if err != nil { fmt.Printf("WaitForEvents failed in kernmulrsymm2dz_async: %+v \n", err) }
}
...
func kernMulC_async(fftM, K *data.Slice, Nx, Ny int) {
	...
	cfg := make3DConf([3]int{Nx, Ny, 1})
	event := k_kernmulC_async(fftM.DevPtr(0), K.DevPtr(0), Nx, Ny, cfg, []*cl.Event{fftM.GetEvent(0), K.GetEvent(0)})

	fftM.SetEvent(0, event)
	K.SetEvent(0, event)
	err := cl.WaitForEvents([]*cl.Event{event})
	if err != nil { fmt.Printf("WaitForEvents failed in kernmulC_async: %+v \n", err) }
}
